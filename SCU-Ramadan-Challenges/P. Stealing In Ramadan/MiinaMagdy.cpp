...
+---------------------------------------------+
|                                             |
|  Copytright, MinaMagdy, 19/03/2023 (16:30)  |
|                                             |
+---------------------------------------------+
*/
/*
...
// ordered map
void rec1(int idx = 0, ll sum = 0, int cnt = 0) {
    if (sum > k || cnt > m) {
        return;
    }
    if (idx == n / 2) {
    ...
    rec1(idx + 1, sum, cnt);
    rec1(idx + 1, sum + A[idx], cnt + 1);
}

void rec2(int idx = n / 2, ll sum = 0, int cnt = 0) {
    if (sum > k || cnt > m) {
        return;
    }
    if (idx == n) {
...
int main(void)
{
    ...
    while (testcase--)
    {
        ...
        for (int i = 0; i < R.size(); i++) {
            for (int j = 0; j < L.size() && i + j <= m; j++) {
                for (ll& x : R[i]) {
                    ll need = k - x;
                    ans += upper_bound(L[j].begin(), L[j].end(), need) - L[j].begin();
                }
            }
        }
        cout << ans << endl;

...
int BFS1() {
    ...
    bool dir;
    while (bfs.size()) {
        curr = bfs.front().ff, dir = bfs.front().ss;
        bfs.pop();

        for (auto &[TO, cost] : adj[curr]) {
            if (cost == dir) {
                if (path[TO][cost] > path[curr][dir])
                    path[TO][cost] = path[curr][dir], bfs.push({TO, cost});
            }
            else {
                if (path[TO][cost] > path[curr][dir] + 1)
                    path[TO][cost] = path[curr][dir] + 1, bfs.push({TO, cost});
            }
        }
    }

    return min(path[to][0], path[to][1]);
}

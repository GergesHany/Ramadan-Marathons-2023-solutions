...
bool validate_q_r(int j) {
    int J = lower_bound(all(pref), pref[j - 1] + q) - pref.begin();
    if (!(J <= n && pref[J] - pref[j - 1] == q))   return false;
    int K = lower_bound(all(pref), pref[J] + r) - pref.begin();
    if (!(K <= n && pref[K] - pref[J] == r))   return false;
    return true;
}
...
void SOLVE(int ts_ctr) {
    ...
    vector<int> End_of_p_segment;
    for (int i = 1, Idx; i <= n; i++) {
        Idx = lower_bound(all(pref), pref[i - 1] + p) - pref.begin();
        if (Idx <= n && pref[Idx] - pref[i - 1] == p)
            End_of_p_segment.push_back(Idx + 1);
    }

    for (auto &Idx : End_of_p_segment)
        if (validate_q_r(Idx))
            return cout << "YES\n", void();
    cout << "NO\n";
}

...
void solve()
{
  ...
  for(int i = 0 ; i < n-1 ; i++){
    ans = 1;
    // count number same adjacency element like [4,4,4,4] or [3,3,3]
    while( i < n and vec[i] == vec[i+1] )  ans ++ , i++;
    
    // take the answer if all elements are equal
    if(ans % 2 == 0)  mx = max(ans , mx);
    else mx = max(ans - 1 ,mx);
   
    // update with pervius step 
    mx = max( min(ans1,ans)*2 , mx );
    // ans1 to save current ans to use in next iteration
    ans1 = ans;
  }
  cout << mx <<ln;
}

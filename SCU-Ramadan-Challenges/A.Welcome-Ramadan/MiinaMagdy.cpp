...
+---------------------------------------------+
|                                             |
|  Copytright, MinaMagdy, 10/10/2022 (16:33)  |
|                                             |
+---------------------------------------------+
*/
/*
...
int main(void)
{
    ...
    while (testcase--)
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (next_greater[i] == -1) {
                ans = max(ans, (ll) i - prev_greater[i]);
            } else if (prev_greater[i] == -1) {
                ans = max(ans, (ll) next_greater[i] - i);
            } else {
                ans = max(ans, (ll) min(next_greater[i] - i, i - prev_greater[i]));
            }
        }
        cout << ans << endl;
    }

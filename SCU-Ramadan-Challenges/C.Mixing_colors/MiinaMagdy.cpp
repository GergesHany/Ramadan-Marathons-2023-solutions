...
ll dp[40][40 * 10][40 * 10];

ll min_cost(ll idx = 0, ll cyan = 0, ll magenta = 0) {
    if (idx == n) return (cyan && magenta && cyan * rb == magenta * ra ? 0 : INF);
    ll &ret = dp[idx][cyan][magenta];
    if (~ret) return ret;
    ret = min_cost(idx + 1, cyan, magenta);
    ret = min(ret, min_cost(idx + 1, cyan + a[idx], magenta + b[idx]) + c[idx]);
    return ret;
}

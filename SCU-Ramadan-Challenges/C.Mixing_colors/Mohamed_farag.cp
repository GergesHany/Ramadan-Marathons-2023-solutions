...
ll solve(ll i,ll x,ll y){
 if(i>=n){
    if((double)x/y==(double)a/b) return 0;
    else return OO;
 }
...
return ret=min(solve(i+1,x,y),solve(i+1,x+ai[i],y+bi[i])+ci[i]);
...
ai.resize(n); bi.resize(n); ci.resize(n);

...
void SOLVE(int ts_ctr) {
    ...
    for (int i = 1, vi; i <= n && cin >> vi; i++) {
        dp[i % 2][0] = dp[i % 2][1] = dp[i % 2][2] = 0;
        dp[i % 2][0] = max(dp[i % 2][0], dp[(i + 1) % 2][0] + vi);

        dp[i % 2][1] = max({dp[i % 2][1], dp[(i + 1) % 2][0] + vi / x, dp[(i + 1) % 2][1] + vi / x});

        dp[i % 2][2] = max({dp[i % 2][2], dp[(i + 1) % 2][1] + vi, dp[(i + 1) % 2][2] + vi});

        maxi = max({maxi, dp[i % 2][0], dp[i % 2][1], dp[i % 2][2]});
    }

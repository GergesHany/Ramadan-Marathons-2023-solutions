...
template < typename myT = int > struct DSU {
    vector < myT > par;
    vector <int> gr_sz;
    int groups;
    ...
    myT getRoot(int node) {return par[node] = (par[node] == node ? node : getRoot(par[node]));}
    int get_size(int node) {return gr_sz[getRoot(node)];}
...
void SOLVE(int ts_ctr) {
    ...
    for (auto &[u, v, d] : vec) {
        ans += d * (ds.get_size(u)) * (ds.get_size(v));
        ds.combine(u, v);
    }

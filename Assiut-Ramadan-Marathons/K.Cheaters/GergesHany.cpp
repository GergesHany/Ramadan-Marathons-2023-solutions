...
void Accepted(){
  ...
  string s;    cin >> s;
  vector < ll > prefix_zero(sz(s) + 1, 0), prefix_one(sz(s) + 1, 0);
  vector < ll > suffix_zero(sz(s) + 1, 0), suffix_one(sz(s) + 1, 0);
  ...
  for (int i = 1; i < sz(s) - 1; i++){
    if(s[i] == '0'){
      ans += prefix_one[i] * suffix_one[i + 1];
    }
    else{
      ans += prefix_zero[i] * suffix_zero[i + 1];
    }
  }

...
+---------------------------------------------+
|                                             |
|  Copytright, MinaMagdy, 26/02/2023 (13:51)  |
|                                             |
+---------------------------------------------+
*/
/*
...
int main(void)
{
    ...
    while (testcase--)
    {
        ...
        while (q--) {
            ...
            int h, m, sec;
            char tmp;
            cin >> d >> h >> tmp >> m >> tmp >> sec;
            int vl = h * 3600 + m * 60 + sec;
            if (vl < mp[d]) {
                cout << mp[d] - vl << endl;
            } else {
                int idx = (find(s.begin(), s.end(), d) - s.begin());
                cout << 86400 - vl + mp[s[(idx + 1) % 7]] << endl;
            }
        }
    }
    return 0;
